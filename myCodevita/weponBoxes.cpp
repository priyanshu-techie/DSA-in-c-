...
int calculatePrice(vector<int>& shifts){
    int sum =0 ;
...
int main(){
    ...
    while(true){
        ...
        list<int> subList(inputArr.begin(),next(inputArr.begin(),N));
        ...
        // recreating the input array
            copyArr.clear();
            copyArr.shrink_to_fit();
            copyArr.push_back(biggestVal);
      		vector<int>::size_type i;
            for( i = N;i<inputArr.size();i++){ copyArr.push_back(inputArr[i]);}
            for( i=0 ;i<forOrder.size();i++){ copyArr.push_back(forOrder[i]);}
            // put the vals in the original
            for(i =0 ;i<copyArr.size();i++){ inputArr[i]=copyArr[i];}
